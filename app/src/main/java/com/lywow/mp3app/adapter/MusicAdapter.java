package com.lywow.mp3app.adapter;
...
public class MusicAdapter extends BaseAdapter {
    ...
    @Override
    public int getCount() {
        return MusicData.musicList.size();
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        //赋值 准确的是绑定赋值的中介
        viewHolder.titleTv.setText(MusicData.musicList.get(position).title);
        viewHolder.artistTv.setText(MusicData.musicList.get(position).artist + "-" + MusicData.musicList.get(position).album);
        viewHolder.albumImgv.setImageBitmap(MusicData.musicList.get(position).albumBip);
